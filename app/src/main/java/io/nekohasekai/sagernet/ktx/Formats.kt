...
private fun JSONObject.optStringOrAny(key: String): String? {
    val norm = key.lowercase(Locale.ROOT).replace("_", "").replace("-", "")
    for (k in keys()) {
        if (k.lowercase(Locale.ROOT).replace("_", "").replace("-", "") == norm) {
...
private fun parseAmneziaAwgLastConfig(lastConfig: String): List<AmneziaWGBean> {
    ...
    val confName = extractConfProfileName(config)
