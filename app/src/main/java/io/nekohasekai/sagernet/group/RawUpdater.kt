...
object RawUpdater : GroupUpdater() {
    ...
    fun parseWireGuard(conf: String): List<WireGuardBean> {
        val confName = extractConfProfileName(conf)
        return parseWireGuard(WireGuardConfParser.parse(conf)).onEach { bean ->
    ...
    fun parseAmneziaWG(conf: String): List<AmneziaWGBean> {
        val confName = extractConfProfileName(conf)
        return parseAmneziaWG(WireGuardConfParser.parse(conf)).onEach { bean ->
            if (bean.name.isBlank() && !confName.isNullOrBlank()) {
                bean.name = confName
            }
