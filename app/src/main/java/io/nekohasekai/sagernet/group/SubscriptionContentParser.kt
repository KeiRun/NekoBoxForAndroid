    ...
    override suspend fun parse(text: String, fileName: String): List<AbstractBean>? {
        ...
        if (WireGuardConfParser.looksLikeWireGuardConf(text)) {
            try {
                ...
                val confName = extractConfProfileName(text)
                profiles.forEach { profile ->
                    if (fileName.isNotBlank()) {
                        profile.name = fileName.removeSuffix(".conf")
                    } else if (profile.name.isBlank() && !confName.isNullOrBlank()) {
                        profile.name = confName
                    }
                }
                return profiles
